    ...
    return (
        <div className="flex flex-1 flex-col gap-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 mb-1">{job.jobRole}</h1>
                    <p className="text-gray-500 flex items-center gap-2">
                        Posted by <span className="font-medium text-indigo-600">{job.comapnyName}</span>
                    </p>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <InfoItem icon={<Briefcase size={18} />} label="Role" value={job.jobRole} />
                            <InfoItem icon={<MapPin size={18} />} label="Location" value={job.location} />
                            <InfoItem icon={<Clock size={18} />} label="Experience" value={`${job.experience} Year(s)`} />
                            <InfoItem icon={<Calendar size={18} />} label="Apply By" value={job.closedAt} />
                        </div>
                        ...
                        <div className="prose prose-sm max-w-none text-gray-600">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {job.techStack.split(',').map((tech, index) => (
                                    <span key={index} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-md text-sm font-medium border border-gray-200">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                                ...
                                <div className="text-gray-900 font-semibold capitalize">{job.interviewType}</div>
                            </div>
                            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                                <div className="flex items-center gap-2 mb-1 text-indigo-700 font-medium">
                                    <Clock size={18} /> Duration
                                </div>
                                <div className="text-gray-900 font-semibold">{job.interviewDuration} Minutes</div>
                            </div>
                            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                                <div className="flex items-center gap-2 mb-1 text-indigo-700 font-medium">
                                    <HelpCircle size={18} /> Questions
                                </div>
                                <div className="text-gray-900 font-semibold">{job.noOfQuestions} Questions</div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div className="flex items-start gap-4 mb-4">
                            ...
                            <div>
                                <h3 className="font-bold text-gray-900">{job.comapnyName}</h3>
                                <a href={job.website} target="_blank" rel="noreferrer" className="text-sm text-indigo-600 hover:underline flex items-center gap-1">
                    ...
                    {/* Recruiter Card */}
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h2 className="text-lg font-semibold text-gray-900 mb-4">Recruiter Contact</h2>
                        <div className="flex items-center gap-3 mb-3">
                            <div className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600">
                                <User size={20} />
                            </div>
                            <div>
                                <p className="font-medium text-gray-900 capitalize">{job.recruiterName}</p>
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-600 p-2 bg-gray-50 rounded-lg">
                            <Mail size={16} />
                            <a href={`mailto:${job.email}`} className="hover:text-indigo-600 transition-colors">{job.email}</a>
                        </div>
                    </div>
